...
func main() {
	...
	dsn := "postgres://user:password@localhost/postgres?replication=database"
	...
	_, err = pglogrepl.CreateReplicationSlot(context.Background(), conn, slotName, outputPlugin, pglogrepl.CreateReplicationSlotOptions{Temporary: false, Mode: pglogrepl.LogicalReplication})
	if err != nil {
	...
	err = pglogrepl.StartReplication(context.Background(), conn, slotName, start, pglogrepl.StartReplicationOptions{PluginArgs: pluginArguments})
	if err != nil {
...
func processV2(walData []byte, relations map[uint32]*pglogrepl.RelationMessageV2, typeMap *pgtype.Map, inStream *bool) {
	...
	case *pglogrepl.UpdateMessageV2:
		log.Println(logicalMsg.NewTuple)
